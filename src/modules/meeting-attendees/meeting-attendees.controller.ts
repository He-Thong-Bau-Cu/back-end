...
export class MeetingAttendeesController {
  ...
  async getById(@Param('id') id: string) {
    try {
      ...
      return BaseResponse.success(resData, "Lấy thông tin người tham gia trong cuộc họp theo ID thành công", HttpStatus.OK);
    } catch (error) {
 ...
 async getAll() {
  try {
    ...
    return BaseResponse.success(resData, "Lấy danh sách người tham gia trong cuộc họp thành công", HttpStatus.OK);
  } catch (error) {
    throw new HttpException(
      { message: error.message },
      HttpStatus.INTERNAL_SERVER_ERROR,
  ...
  async create(@Body() createMeetingAttendeeDto: CreateMeetingAttendeeDto) {
    try {
      ...
      return BaseResponse.success(resData, "Tạo người tham gia trong cuộc họp thành công", HttpStatus.OK);
    } catch (error) {
      throw new HttpException(
        { message: error.message },
        HttpStatus.INTERNAL_SERVER_ERROR,
  ...
  async updateStatusAttendance(
    ...
    try {
      ...
      return BaseResponse.success(resData, "Cập nhật trạng thái tham gia cuộc họp thành công", HttpStatus.OK);
    } catch (error) {
      throw new HttpException(
        { message: error.message },
        HttpStatus.INTERNAL_SERVER_ERROR,
  ...
  @Put(':id')
   ...
   async update(@Param('id') id: string, @Body() updateMeetingAttendeeDto: UpdateMeetingAttendeeDto) {
     try {
       ...
       return BaseResponse.success(resData, "Cập nhật thông tin người tham gia trong cuộc họp thành công", HttpStatus.OK);
     } catch (error) {
       throw new HttpException(
         { message: error.message },
         HttpStatus.INTERNAL_SERVER_ERROR,
