...
export class ElectionsService {
  constructor(
    ...
    @InjectModel(ElectionDocuments.name) private readonly electionDocumentsModel: Model<ElectionDocuments>
  ) {}
  ...
  async searchElectionDocumentsByElectionId(electionId: string){
    try{
      const documents = await this.electionDocumentsModel.find({electionId: new Types.ObjectId(electionId)}).populate('electionId').exec();
      return documents;
    }catch(error){
      throw error;
    }
  ...
  async updateElections(id: string, data: ElectionsDto){
    try{
      const election = await this.electionsModel.findByIdAndUpdate(new Types.ObjectId(id), data, {new: true}).exec();
      return election;
    }catch(error){
      throw error;
    }
  ...
  async searchDocumentsByElectionId(electionId: string){
    try{
      const documents = await this.electionDocumentsModel.find({electionId: new Types.ObjectId(electionId)}).exec();
      return documents;
    }catch(error){
      throw error;
    }
  ...
  async deleteDocumentByElectionId(electionId: string){
    try{
      const documents = await this.electionDocumentsModel.deleteMany({electionId: new Types.ObjectId(electionId)}).exec();
      return documents;
    }catch(error){
      throw error;
    }
