...
export class DelegationsController {
  ...
  @ApiOperation({ summary: 'Lấy tổng hợp dữ liệu ủy quyền theo id của cuộc bầu cử' })
  @ApiResponse({ status: 200, description: 'Lấy tổng hợp dữ liệu tất cả ủy quyền thành công' })
  @ApiResponse({ status: 500, description: 'Dữ liệu không hợp lệ' })
  @Get('summary/preside/election/:id')
  async getSummaryByElectionId(@Param('id') id: string) {
    const resData = await this.delegationsService.getSummaryDelegatesByElectionId(id);
    return BaseResponse.success(resData, MESSAGE.SUCCESS, HttpStatus.OK);
  }
