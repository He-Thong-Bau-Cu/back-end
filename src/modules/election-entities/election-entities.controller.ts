...
export class ElectionEntitiesController {
  ...
  @ApiResponse({ status: 500, description: 'Lỗi server' })
  async create(
    @Body() createElectionEntity: CreateElectionEntityDto,
    @Req() req: CustomRequest): Promise<BaseResponse> {
    try {
      const resData = await this.electionEntitiesService.create(createElectionEntity, req.user.sub);
  ...
  async update(
    @Param('id') id: string, 
    @Body() updateElectionEntity: UpdateElectionEntityDto,
    @Req() req: CustomRequest): Promise<BaseResponse> {
    try {
      const resData = await this.electionEntitiesService.update(id, updateElectionEntity, req.user.sub);
